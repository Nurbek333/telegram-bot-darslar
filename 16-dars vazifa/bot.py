...
async def get_phone_number(message:Message,state:FSMContext):
    ...
    if re.match(pattern,message.text):
        ...
        await state.set_state(Form.yosh_1)
...
@dp.message(Form.yosh_1, F.text)
async def get_yosh_1(message:Message,state:FSMContext):

    yosh_1 = message.text
    await state.update_data(yosh_1=yosh_1)
...
async def get_address(message:Message,state:FSMContext):
    ...
    yosh_1 = data.get("yosh_1")
    telegram_id = message.from_user.id
    try:
        connection = sqlite3.connect("sqlite.dp")
        cursor = connection.cursor()
        command = f"""
            INSERT INTO USERS('first_name','last_name','phone_number','telegram_id','my_photo','address','yosh_1')
            VALUES('{first_name}','{last_name}','{phone_number}','{telegram_id}','{my_photo}','{address}','{yosh_1}')
        """
